...
public class Topic extends Node {
  ...
  public Topic(String name, String ipAddr, int port) {
    super("", ipAddr, port);
  ...
  public void writeMetadata() {
    try {
      ...
      File yourFile = new File(Topics.ROOT_FOLDER_NAME + name + "/metadata.json");
  ...
  public void readMetadata() {
    try {
      Gson gson = new Gson();
      Files.createDirectories(Paths.get(Topics.ROOT_FOLDER_NAME + name));
      File yourFile = new File(Topics.ROOT_FOLDER_NAME + name + "/metadata.json");
      yourFile.createNewFile();
  ...
  public int noOfMessagesForTopic() {
    return new File(Topics.ROOT_FOLDER_NAME + name).list().length;
  }

  public void addMessage(Message m) {
    try {
      Files.createDirectories(Paths.get(Topics.ROOT_FOLDER_NAME + name));
      File yourFile1 = new File(Topics.ROOT_FOLDER_NAME + name + "/metadata.json");
      yourFile1.createNewFile();
      int newMessageId = noOfMessagesForTopic();
      System.out.println(
          "file location: " + Topics.ROOT_FOLDER_NAME + name + "/" + newMessageId + ".json");
      File yourFile = new File(Topics.ROOT_FOLDER_NAME + name + "/" + newMessageId + ".json");
      yourFile.createNewFile();
      FileOutputStream oFile = new FileOutputStream(yourFile, false);
      Gson gson = new Gson();
  ...
  public List<Message> getMessagesSubList(int from) {
    ...
    try {
      ...
      while (true) {
        File yourFile = new File(Topics.ROOT_FOLDER_NAME + name + "/" + id + ".json");
        FileInputStream iFile = new FileInputStream(yourFile);
        String data = new String(iFile.readAllBytes());
